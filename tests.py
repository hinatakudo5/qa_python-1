...
def test_set_book_genre_does_not_set_genre_for_unknown_book():
    ...
    books = collector.get_books_genre()
    # книга так и не появилась в словаре genres
    assert unknown_book not in books


def test_get_book_genre_returns_none_for_book_without_genre():
    collector = BooksCollector()
...
def test_get_books_for_children_excludes_books_with_age_rating():
    collector = BooksCollector()

    # жанр без возрастного рейтинга
    safe_genre = next(
        genre for genre in collector.genre
        if genre not in collector.genre_age_rating
    )
    # жанр с возрастным рейтингом
...
def test_add_book_in_favorites_adds_only_existing_book_and_ignores_duplicates():
    ...
    # сначала добавляем книгу в общий список
    collector.add_new_book(book_name)
